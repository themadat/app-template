(function () {
  ...
  const CONFIG = {
    ...
    help: [
      ...
      { id: "icons", title: "Finding and copying icons", section: "Features", keywords: "icons svg symbol search semantic tags categories accessibility editing text formatting games locations maps health keyboard math media nature privacy security rays sparkles cloud drive shapes badged time transportation weather plus minus checkmark xmark squared circled slashed quick select collapse copy clipboard rename right click remove undo source filter resize divider catalog compiler arrows", html: "<p>Search by name, alias, semantic tag, category, Symbol/Custom type, repository, or source metadata; multiple search words must all match. The desktop rail stays visible while results scroll and separates semantic <strong>What it is</strong> categories from the smaller <strong>How it looks</strong> set: Badged, Circled, Squared, Slashed, Shapes, and Rays &amp; Sparkles. Category branches can be collapsed and their state is remembered. Use Up and Down Arrow while a category is focused to activate the adjacent category. Drag the divider to change the rail width, or focus it and use Left/Right, Home, or End; the width is remembered. Results stay alphabetical and appear 500 at a time. Press <kbd>Enter</kbd> to move to the grid, choose a preview to copy its sanitized SVG, select its name to edit metadata, right-click it to remove it from the selected category with Undo, or use the information button for tags, original source details, and the editable filter source. Use <kbd>F</kbd> for categories and filters, <kbd>G</kbd> for the first result, <kbd>I</kbd> for focused-icon details, <kbd>C</kbd> to clear search, and <kbd>L</kbd> to show more. The committed catalog is rebuilt with <code>build/compile-icon-library.mjs</code>.</p>" },
      { id: "icon-overrides", title: "Icon overrides", section: "Features", keywords: "icon rename type custom symbol groups source right click remove undo override export json compiler feed back developer", html: "<p>Open Icon details to edit Name and Type immediately, or select an icon name to edit Name, Type, groups, and the source used by repository filtering while retaining every original file reference. When a category is selected, right-click an icon to remove it from that group and use Undo if needed. Choose <strong>Export overrides</strong> in the editor or Developer Mode to download a compact array containing icon IDs, labels, category IDs, and optional type and source values. Attach that file in a future request to hard-code the changes, or use it directly as <code>build/icon-library-overrides.json</code>; the compiler also accepts the wrapped format, whose optional <code>excludedIconIds</code> list permanently rejects unwanted extractions.</p>" },
