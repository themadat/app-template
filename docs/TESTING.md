...
- [ ] Header, version/Beta pills, centered global search, toolbar, icon catalog, and combined top-bar storage/sync status fit without horizontal overflow.
...
- [ ] A waiting service worker changes the top-bar Update icon to red with an accessible available-update label, without a persistent pop-up. Update saves local edits, checks/installs the worker, and refreshes. Offline or failed local saves prevent refresh and explain the problem.
- [ ] What’s New defaults to 20 seconds. Settings accepts 1–300 whole seconds, persists the device preference, and restarts an unread countdown when changed; invalid input is rejected. Cloud downloads preserve this preference.
