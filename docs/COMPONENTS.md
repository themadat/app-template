...
Catalog cards deliberately show only the SVG preview and multiline name, with Custom/Symbol at bottom left and a compact information button at bottom right. Selecting the card copies its complete sanitized SVG. The full-width catalog uses a compact sticky vertical rail for result count, type, source, and persistent multi-category chips, including Cloud/Server, Health, Nature, Rays, Shapes, Badged with 39 nested suffix-specific choices (including plain Badge), Squared, Circled, Slashed, Sparkled, Time, and Weather. A pointer-, touch-, and keyboard-operable divider changes the rail width and persists it locally; the mobile layout keeps horizontal category chips and hides the divider. Up and Down Arrow activate the adjacent category, while the card grid’s Arrow keys, Home, and End move among visible copy buttons. Results remain alphabetical and render in batches of 500. Global search matches names, aliases, categories, semantic tags, repositories, and source metadata. The information dialog shows categories, tags, normalized identifier, aliases, repositories, filenames, full paths, and source-symbol names. Its Edit name & groups action opens a native metadata dialog with a validated name, multi-group checkboxes, compiled-value reset, and export of all local changes as a compact compiler-ready JSON array. Developer Mode adds a persistent minimum-label-character filter and another override-export action.
