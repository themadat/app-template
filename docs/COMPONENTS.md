...
The top-left app icon uses separate light and dark SVG artwork with a prominent X and three concentric circles over a deliberately quieter construction grid at 42px. Clicking or pressing `T` changes the color theme and therefore swaps the matching icon artwork; the button only scales slightly while actively pressed. The favicon uses three grid lines per axis plus an X and two circles—without square outlines—in neon blue on the full-bleed Safari-gray background.
