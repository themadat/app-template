...
Catalog cards deliberately show only the SVG preview and multiline name, with Custom/Symbol at bottom left and the information button at bottom right. Selecting the card copies its complete sanitized SVG. The full-width catalog uses a compact sticky vertical rail for result count, type, source, and persistent multi-category chips, including Cloud/Server, Shapes, Badged, Squared, Circled, Slashed, and Sparkled; results remain alphabetical. Global search matches names, aliases, categories, semantic tags, repositories, and source metadata. The information dialog shows categories, tags, normalized identifier, aliases, repositories, filenames, full paths, and source-symbol names. Arrow keys, Home, and End move among visible card copy buttons.
