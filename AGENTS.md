...
- Preserve the focused foundation: top bar with centered search, single Notes modal, demonstrative Roadmap without a redundant navigation strip, Settings, combined floating storage/sync status, local persistence/recovery, and optional GitHub Sync. Do not restore the removed Records interface, multi-note workspace, or rich-text editor without an explicit request.
