...
The template starts on the pre-launch `0.0.1` line at version `0.0.1.77` (`major.minor.patch.build`). Routine updates increment the fourth number.
...
- Full-width responsive catalog of 7,232 deduplicated SVG icons gathered from the configured local sources, including 7,152 source files for each native Ultra, Light, Medium, Bold, and Black set.
...
Create a separate GitHub repository containing this template, then clone the **new repository** using SSH. Follow the [step-by-step SSH clone instructions](docs/RESET.md#prepare-the-new-repository-with-ssh), including the personal/work account setup when needed. Open the new checkout in Codex and say `reset`. The workflow asks you to confirm the app name and provide a high-quality square SVG or PNG app icon before it edits anything. It derives and autofills the short name, slug, page and manifest titles, download/sync names, storage namespace, and other identity-bearing copy from the confirmed name.
...
Provide the app name, description, icon, application repository URL, and sync owner/repository/branch/JSON path in your initial reset request; the [setup values and copyable request](docs/RESET.md#values-to-provide-at-the-beginning) list each one. The agent sets the application repository in `config.identity.repository` and the fixed sync target in `config.cloudSync` inside `assets/js/config.js`. Then open **Settings → Data Sync** and provide a fine-grained personal access token limited to the sync repository (`app-data`) with **Contents: Read and write** permission. The GitHub heading links to the application repository. Owner, linked sync repository, branch, and linked JSON file path appear together on one read-only metadata line so saved or imported application state cannot redirect synchronization.
