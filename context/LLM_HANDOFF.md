...
This repository is a focused SVG icon-library application. It includes the reusable top bar with centered icon search, a searchable and copyable 4,042-icon main catalog with wider uniform-font SF Symbols/Custom cards and editable source filters, 500-icon batches, directly editable icon names/types/groups, identical Name/Type field layouts across metadata entry points, selected-category right-click removal with Undo, compact override export, and a sticky resizable category rail. The rail separates semantic **What it is** destinations—including Celebrations & Awards, Clothing & Personal Items, Education & Science, Home & Appliances, and Sports & Recreation—from the smaller **How it looks** set (Badged, Building, Circled, Squared, Slashed, Shapes, and Rays & Sparkles); Other remains visible directly below All at zero, every retained icon has at least one group, all 1,499 Objects & Tools icons have an additional semantic destination, Animals & Plants and Weather nest under Nature, Maps/Maps & Travel are consolidated into Locations, Text Formatting nests under Editing, and Connectivity joins Devices. Matching baked metadata is automatically removed from device-local pending overrides. The compiler excludes the broad `!backups:data` parent while reading only its configured Objects & Tools child, retains the existing compiled catalog when former source folders move, and preserves unique stable IDs for distinct artwork sharing a source name. The app also retains simplified Appearance settings, Developer Mode filters and divider feedback, Notes, Settings/Roadmap, combined local/GitHub Sync status, persistence/recovery, install assets, and the offline shell. The removed Records interface, multi-note workspace, rich-text editor, and app-space Roadmap are not part of the template.
