...
Cloud Sync now lives in the top toolbar beside Update, Notes, and Settings. Update saves local changes, checks/installs the service worker, and force refreshes. Available updates switch to the red updateReady symbol and accessible label without a persistent update pop-up. The What’s New timer uses device-only preferences.controls.whatsNewDismissSeconds (20 by default, normalized to 1–300); changing it restarts an unread banner countdown.
...
The supplied String Lights artwork is retained at `assets/custom-symbols/string-lights.svg` as Custom, with theme-aware `currentColor` fill. Its baked metadata places it in Home & Appliances and Celebrations & Awards.
